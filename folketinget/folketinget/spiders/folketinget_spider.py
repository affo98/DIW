...
class FolketingetSpider(scrapy.Spider):
    ...
    'FEEDS': {
        ...
        'data_speech.csv': {
            'format': 'csv',
            'overwrite': True,
            'fields': ['meeting_id','agenda_item_id', 'speech_item_id', 'time_start', 'time_end', 'speaker_name', 'speaker_party', 'speaker_role', 'speaker_title', 'speech_item_text'],
        }
    ...
    def seperate_agenda_items(self, agenda_items_seperators):
        ...
        agenda_items = [BeautifulSoup(agenda_item, 'html.parser') for agenda_item in agenda_items]
        return agenda_items
    ...
    def seperate_agenda_item_speech(self, agenda_item_seperators):
        ...
        agenda_item_speeches = [BeautifulSoup(item, 'html.parser') for item in agenda_item_speeches]
    ...
    def seperate_agenda_item_questions(self, agenda_item_question_seperators):
        ...
        agenda_item_questions = [BeautifulSoup(item, 'html.parser') for item in agenda_item_questions]
    ...
    def find_data_speech(self, speech_item):
        ...
        speaker_party = speech_item.select_one('meta[name="GroupNameShort"]').get('content')
        speaker_role = speech_item.select_one('meta[name="OratorRole"]').get('content')
        
        try:
            speaker_title = speech_item.select_one('p[class="TalerTitel"]').get_text(strip=True) 
        except AttributeError:
            speaker_title = None
        
        if speaker_title is None:    
            try:
                speaker_title = speech_item.select_one('p[class="TalerTitelMedTaleType"]').get_text(strip=True)
            except AttributeError:
                speaker_title = None
  
        speech_item_text = [e.get_text() for e in speech_item.find_all('p', class_=['Tekst', 'TekstIndtryk', 'TekstLuft'])]
        speech_item_text = ' '.join(speech_item_text)
        return time_start, time_end, speaker_name, speaker_party, speaker_role, speaker_title, speech_item_text
    ...
    def write_data_speech(self, meeting_id, agenda_item_id, speech_item_id, time_start, time_end, speaker_name, speaker_party, speaker_role, speaker_title, speech_item_text):
        d = SpeechItem()
        d['meeting_id']  = meeting_id
        d['agenda_item_id'] = agenda_item_id
    ...
    def parse_htm_file(self, response):
        ...
        for agenda_item in agenda_items:
            try: 
                agenda_item_number = agenda_item.select_one('meta[name="ItemNo"]').get('content')
            except AttributeError:
            ...
            if agenda_item_number != "0" and agenda_item_number is not None:
                ...
                if "(spørgetid)" not in agenda_title and "(spørgetime)" not in agenda_title: 
                    ...
                    agenda_item_seperators_list = agenda_item.find_all('meta', attrs={'name': 'Start MetaSpeakerMP'})
                    agenda_item_speeches = self.seperate_agenda_item_speech(agenda_item_seperators_list)
                    
                    self.speech_item_id = 1
                    for speech_item in agenda_item_speeches:
                        time_start, time_end, speaker_name, speaker_party, speaker_role, speaker_title, speech_item_text = self.find_data_speech(speech_item)
                        
                        yield from self.write_data_speech(self.meeting_id, agenda_item_id, self.speech_item_id, time_start, time_end, speaker_name, speaker_party, speaker_role, speaker_title, speech_item_text)
                        
                    type = '0'
                    agenda_title = self.find_data_agenda(agenda_item)
                    yield from self.write_data_agenda(self.meeting_id, agenda_item_id, agenda_title, type)
                    agenda_item_id += 1
                        

                #check if agenda_item is 'spørgetime' or 'spørgetid'
                if any(keyword in agenda_title for keyword in ("(spørgetid)", "Spørgetid", "(spørgetime)", "Spørgetime")):
                    #seperate into each question
                    agenda_item_question_seperator_list = agenda_item.find_all('meta', attrs={'name': 'SubItemNo'})
                    ...
                    for question in agenda_item_questions:
                        ...
                        question_item_seperators_list = question.find_all('meta', attrs={'name': 'Start MetaSpeakerMP'})
                        ...
                        type = '1' if any(keyword in agenda_title for keyword in ("(spørgetid)", "Spørgetid")) else '2' 
